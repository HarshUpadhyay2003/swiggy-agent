...
class CartService:
    """In-memory cart management with shared state across instances and totals calculation."""

    # Shared class-level storage across all CartService instances
    _shared_carts: Dict[str, Dict[str, Any]] = {}
    ...
    def __init__(self, catalog_service: Optional[CatalogService] = None, order_service: Optional[OrderService] = None) -> None:
        ...
        # Point instance carts to shared class storage
        self.carts = CartService._shared_carts
    ...
    def add_to_cart(self, session_id: str, item: Dict[str, Any], quantity: int = 1) -> Dict[str, Any]:
        ...
        logger.info(f"[CART_SERVICE] Added {quantity}x item_id={item['item_id']} to session '{session_id}'")
        return cart
    ...
    def remove_from_cart(self, session_id: str, item_id: int, quantity: Optional[int] = None) -> Dict[str, Any]:
        ...
        if not item:
            raise ValueError(f"Item {item_id} not found in cart for session '{session_id}'")
